...
const { Band, Musician, Instrument, Sequelize } = require("./db/models");
...
app.get("/musicians", async (req, res, next) => {
  ...
  if (firstName || lastName) {
    query.where = { ...query.where, firstName, lastName };
  }
  ...
  if (name) {
    query.include.push({ model: Band, where: { name } });
  }
  ...
  const type = req.query.instrumentTypes;
